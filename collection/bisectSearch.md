...
3. 理解记忆：
   ...
   - 与 **二分查找左（右）侧插入点** 算法的不同之处 ①：**插入点** 和 **边界** 的细微不同。对于左侧来说，左边界的索引和左侧插入点是一致的，所以返回的还是 left；对于右侧来说，右边界的索引应该是右侧插入点的索引 - 1，所以返回的是 left-1。

     > 比如在 [1,2,3,3] 中查找目标值 3，右侧插入点的索引是 4，但右边界的索引是 3。
     
   - 与 **二分查找左（右）侧插入点** 算法的不同之处 ②：需要增加 **判断条件** 用于判断原序列中 x 是否存在：如果 x 存在，那么边界索引对应的元素必然等于 x，返回边界索引 left 或 left-1；否则说明 x 不存在，返回 -1。需要注意的是，在二分查找左边界的算法中，如果目标值 x 不存在且大于原序列的最大值，那么 left 会溢出，所以需要额外处理。
